...
btnButton.onclick = function(){
  console.log('.btn요소 중 첫 번째 요소가 클릭되었습니다.');
}
/*2. document.querySelectAll(선택자)
여러 개의 요소 참조를 모두 가져와 한번에 이벤트 핸들러를 등록한다.*/

const divs = document.querySelectorAll('div');
divs.forEach(div => div.onclick = function() {
  //클릭 이벤트가 일어난 요소 그 자체를 의미한다.
  this.style.backgroundColor = randomColorFunc();
});

//#3) addEventListener 메서드
/*표준 이벤트 모델로서 HTML요소에 addEventListener 메서드를 사용하여 이벤트를 등록한다
한 요소에 여러개의 이벤트 핸들러 등록이 가능하다.*/
const btnsButton = document.querySelectorAll('.btnsChange');
btnsButton.forEach(btn => {
  //btn은 DOM요소이다.(HTML요소를 객체로 변환한다)
...
// *함수 호출 vs 함수 등록
//&함수 호출 : 함수명() >> 코드를 읽는 즉시 함수가 실행됨 | 그러나 함수 등록 : 함수명 >> 부가적인 발생으로 함수를 전달하여 실행한다.
